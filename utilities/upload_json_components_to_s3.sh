 ...
 # Get all component names from the JSON file
    ...
    for component in $components; do
        ...
        for file in $files; do
            ...
            aws s3 cp "${component}/$filename" "s3://${S3_BUCKET}/${s3_app_folder_name}/${filename}"
            if [ $? -ne 0 ]; then
        ...
        if [[ -n "$dag_files" ]]; then
            ...
            for dag_file in $dag_files; do
                aws s3 cp "${component}/$dag_file" "s3://$S3_BUCKET/$s3_app_folder_name/$dag_file"
                if [ $? -ne 0 ]; then
